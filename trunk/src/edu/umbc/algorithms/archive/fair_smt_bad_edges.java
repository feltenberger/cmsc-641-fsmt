 ...
 *
 */
public class fair_smt_bad_edges extends JPanel implements Runnable {
	 ...
	 */
	public fair_smt_bad_edges(int width, int height, int numNonSteinerNodes) {
		this.width = width;
		this.height = height;
	...
	private void generateRandomNonSteinerNodes() {
		...
		for (int k = 0; k < numNonSteinerNodes; k++) {
			...
			p.w = Math.asin((y(k) - 0.0) / euclideanDistance(x(k), y(k), 0.0, 0.0)) + half_pi;
			this.points.add(p);
		}
	}
	...
	public void convexHull() {
		...
		while (lto != x2(0)) {
			...
			for (int i = 1; i < numNonSteinerNodes; i++) {
				w(i, Math.asin((y(i) - yy) / euclideanDistance(x(i), y(i), xx, yy)) + half_pi);
				if (x(i) > xx) {
					w(i, 3 * Math.PI / 2 - Math.asin((y(i) - yy) / euclideanDistance(x(i), y(i), xx, yy)));
				}
				w(i, w(i) + Math.PI);
				if (w(i) > 2 * Math.PI) {
					w(i, w(i) - 2 * Math.PI);
	...
	void mst() {
		...
		// lock down the object while we're modifying the edges list...
		synchronized(this) {
			...
			for (int i = 0; i < count; i++) {
				...
				if (label == 0) {
					// get the adjoining points
					Point p1 = points.get((int)ei[i]);
					Point p2 = points.get((int)ej[i]);
					// create a new edge
					Edge edge = new Edge(p1, p2);
	
					// set pointers to indexes in points list b/c the japanese dude did...
					...
					// connect edges -- whatever this means.
	...
	private double totalTreeLength() {
		...
		for (int i = 0; i < numNodesIncludingSteiner - 1; i++) {
			...
			total = total
					+ euclideanDistance(x(e.index1), y(e.index1), x(e.index2),
							y(e.index2));
		}
		return total;
	}
	...
	private void hantei() {
		...
		if (treeLen < minTreeLen) {
			...
			*/
			...
			for (int i = 0; i < numNodesIncludingSteiner - 1; i++) {
				//e1min[i] = e1[i];
				e1min[i] = this.edges.get(i).index1;
				//e2min[i] = e2[i];
				e2min[i] = this.edges.get(i).index2;
			}
	...
	public void paint(java.awt.Graphics g) {
		...
		for (int i = 0; i < numNodesIncludingSteiner - 1; i++) {
			Point p1 = this.edges.get(i).p1;
			Point p2 = this.edges.get(i).p2;
			//g.drawLine((int) x(e1[i]), (int) y(e1[i]), (int) x(e2[i]), (int) y(e2[i]));
		...
		synchronized(this) {
			...
			for (int i = 0; i < numNodesInSMT - 1; i++) {
				...
				Point p2 = this.minPoints.get(e2min[i]);
				g.drawLine(p1.xInt(), p1.yInt(),
						p2.xInt(), p2.yInt());
			}
		}
	...
	public static void main(String...args) {
		fair_smt_bad_edges ms = new fair_smt_bad_edges(800, 800, 100);
		JFrame f = new JFrame();
