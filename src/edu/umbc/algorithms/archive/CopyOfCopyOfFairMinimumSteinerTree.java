 ...
 * 
 */
public class CopyOfCopyOfFairMinimumSteinerTree extends Applet implements Runnable {
	private static final long serialVersionUID = 1L;
	private Thread th;
	private Color bgColor;
	private double pi;
	private int tim, timlabel, mouselabel, counter2;
		...
		if (tx < x1[mij3]) {
			x1[NN] = (-an * bn + Math.pow(an * an * bn * bn - (bn * bn - rr * rr)
					* (1.0 + an * an), 0.5))
					/ (1.0 + an * an)
					+ 0.0001
					* Math.cos(pi * 2.0 * Math.random());
			y1[NN] = an * x1[NN] + bn + 0.0001
					* Math.sin(pi * 2.0 * Math.random());
			sl[NN] = 1;
			NN++;
		} else {
			x1[NN] = (-an * bn - Math.pow(an * an * bn * bn - (bn * bn - rr * rr)
		...
		while (tlabel == 0) {
			...
			for (int it1 = numPoints; it1 < NN; it1++) {
				if (cn[it1] == 3) {
					...
					qw = Math.acos((qb * qb + qc * qc - qa * qa)
							/ (2.0 * qb * qc));
					qwe = euclDist(qw, 0.0, 2.0 * pi / 3.0, 0.0);
					maxq = qwe;
					xx = x1[connect[it1][1]];
					yy = y1[connect[it1][1]];
					xx2 = x1[connect[it1][2]];
