 ...
 *
 */
public class MinSteinerApplet extends Applet implements Runnable {
	 ...
	 */
	private int numNonSteinerNodes;
	/**
	...
	public void init() {
		...
		// the number of nodes
		numNonSteinerNodes = Integer.parseInt(getParameter("numNodes"));
	}
	...
	private double totalTreeLength() {
		...
		int mokuk;
		mokukan = 0.0;
		for (mokuk = 0; mokuk < numNodesIncludingSteiner - 1; mokuk++) {
			mokukan = mokukan
					+ euclideanDistance(x1[e1[mokuk]], y1[e1[mokuk]], x1[e2[mokuk]],
							y1[e2[mokuk]]);
		}
		return mokukan;
	}
	...
	private void bohe() {
		...
		numNodesIncludingSteiner = shinNN;
		for(int hi = numNonSteinerNodes; hi < numNodesIncludingSteiner; hi++) {
			isSteinerNode[hi] = 1;
		}
	}// bohe
	...
	private void addSteinerNodeToGraph(int mij1, int mij2, int mij3) {
		...
		if (steinerX < x1[mij3]) {
			x1[numNodesIncludingSteiner] = (-an * bn + Math.pow(an * an * bn * bn
					- (bn * bn - distToMidpoint * distToMidpoint) * (1.0 + an * an), 0.5))
					/ (1.0 + an * an)
					+ 0.0001
					* Math.cos(Math.PI * 2.0 * Math.random());
			y1[numNodesIncludingSteiner] = an * x1[numNodesIncludingSteiner] + bn + 0.0001
					* Math.sin(Math.PI * 2.0 * Math.random());
			isSteinerNode[numNodesIncludingSteiner] = 1;
		...
		// now re-adjust the three points and the steiner point by the average again
		x1[mij1] = x1[mij1] + avgX;
		y1[mij1] = y1[mij1] + avgY;
		x1[mij2] = x1[mij2] + avgX;
		y1[mij2] = y1[mij2] + avgY;
		...
		// adjust the new steiner node by the average as well!
		x1[numNodesIncludingSteiner - 1] = x1[numNodesIncludingSteiner - 1] + avgX;
		y1[numNodesIncludingSteiner - 1] = y1[numNodesIncludingSteiner - 1] + avgY;
		*/
		msbn2(numNodesIncludingSteiner, mij1, mij2, mij3);
		numNodesIncludingSteiner++;
	}// addSteinerNodeToGraph

	private void msbn2(int steinerIndex, int mij1, int mij2, int mij3) {
		double avgX, avgY, an, bn, distToMidpoint;
				...
				if (cn[i] == 3) {
					...
					if (maxq > Math.PI / 180.0) {
						...
						computeSteinerPoint(connect[i][0], connect[i][1],
								connect[i][2]);
						msbn2(i, connect[i][0], connect[i][1],
								connect[i][2]);
						mst();
						hantei();
					}// maxq>pi/180.0
	...
	public void run() {
		...
		for (int k = 0; k < numNonSteinerNodes; k++) {
			...
			isSteinerNode[k] = 0;
			w1[k] = Math.asin((y1[k] - yy) / euclideanDistance(x1[k], y1[k], xx, yy))
					+ half_pi;
		}
