 ...
 * 
 */
public class CopyOfFairMinimumSteinerTree extends Applet implements Runnable {
	private static final long serialVersionUID = 1L;
	private Thread th;
	private Color bgColor;
	private int numIterationsSinceLastClick, mouseWasClicked, counter2;
	private int numPoints, height, width, label, h, ni, nj, minNN;
	...
	public void init() {
		...
		colors[2] = Color.cyan;
		/*
		 * <param name="height" value="350"> <param name="width" value="500">
		 */
	...
	private void mst() {
		...
		while (tyao == 0 && mstcou < 2) {
			...
			koko = 0;
			for (int i = 0; i < co; i++) {
				if (mouseWasClicked == 1 && mstcou == 0) {
					tyao = 0;
					mstcou++;
					break;
				}
				if (mouseWasClicked == 1 && mstcou == 1) {
					tyao = 0;
					mstcou++;
				}
				label = 0;
	...
	private void musoubana(int mij1, int mij2, int mij3) {
		...
		bn = ty - an * tx;
		if (tx < x1[mij3]) {
			x1[NN] = (-an * bn + Math.pow(an * an * bn * bn
					- (bn * bn - rr * rr) * (1.0 + an * an), 0.5))
					/ (1.0 + an * an)
					+ 0.0001
	...
	private void msbn2(int msbn, int mij1, int mij2, int mij3) {
		...
		bn = ty - an * tx;
		if (tx < x1[mij3]) {
			x1[msbn] = (-an * bn + Math.pow(an * an * bn * bn
					- (bn * bn - rr * rr) * (1.0 + an * an), 0.5))
					/ (1.0 + an * an)
					+ 0.0001
					* Math.cos(Math.PI * 2.0 * Math.random());
	...
	private void hontai() {
		...
		// keep running this loop until the mouse is clicked!
		while (mouseWasClicked == 0) {
			numIterationsSinceLastClick++;
			int timlabel = 0;
			if (numPoints > 20) {
				if (numIterationsSinceLastClick % 3 == 0) {
					timlabel = 1;
				}
			}
			if (numPoints > 9 && numPoints < 21) {
				if (numIterationsSinceLastClick % 30 == 0) {
					timlabel = 1;
				}
			}
			if (numPoints < 10) {
				if (numIterationsSinceLastClick % 100 == 0) {
					timlabel = 1;
				}
			}
			if (timlabel == 1) {
	...
	public void paint(java.awt.Graphics g) {
		...
		g.drawString("counter=" + numIterationsSinceLastClick, 65, 30);
		if (numPoints < 2) {
