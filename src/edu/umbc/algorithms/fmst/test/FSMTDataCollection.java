...
public class FSMTDataCollection {
	...
	private static final String HEADER = "run num	std dev	max pcr	min pcr	avg pcr	total pcr	k	target pcr";
	/**
	...
	private FairSMT generateFSMT(int heightAndWidth, int numNodes, double pcrGoal, int k) {
		FairSMT fsmt = new FairSMT(heightAndWidth, heightAndWidth, numNodes);
			...
			for(int k = minK; k <= maxK; k++) {
				for(int nn = minNumNodes; nn <= maxNumNodes; nn += numNodeIncrement){
					for(int run = 0; run < runsPerSize; run++) {
						try {
							FairSMT fsmt = this.generateFSMT(hw, nn, 0.0, 5);
							this.runFSMT(fsmt);
	...
	public static void main(String...args) throws Exception {
		...
		//	number of nodes from 10 to 50 (increments of 5)
		//		from k=1 to 20
		//			run each configuration 10 times
		//dataCollector.runTest(100, 600, 200, 50, 50, 5, 1, 5, 3);
		dataCollector.runTest(100, 600, 100, 10, 50, 5, 1, 20, 3);
	}
}
